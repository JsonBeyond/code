...
/**
 * @ClassName HttpSyncClient
 * @Description 同步的HTTP请求对象，支持post与get方法以及可设置代理
 * @Author Alex
 * @CreateDate 2019/5/14 9:49
 * @Version 1.0
 */
public class HttpSyncClient {
    ...
    public String httpGet(String url, List<BasicNameValuePair> parameters) {
        ...
        try {
            ...
            logger.error(Thread.currentThread().getName()
                    + "httpGet Send Error,URISyntax error:" + e.getMessage());
        } finally {
            // 释放连接,将连接放回到连接池
    ...
    public String httpPost(String url, List<BasicNameValuePair> parameters,
        ...
        try {
            ...
            logger.error(Thread.currentThread().getName()
                    + "httpPost Send Error,URISyntax error:" + e.getMessage());
        } finally {
            // 释放连接,将连接放回到连接池
